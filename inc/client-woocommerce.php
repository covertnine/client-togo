...
function c9_add_woocommerce_icon( $items, $args ) {
	...
	if ( defined('WC_VERSION') ) { //show cart contents if woo is active
		...
		if ( 'primary' == $args->theme_location ) {
			...
			if ( $count != 0 ) {
				$items .= '<li itemscope="itemscope" class="nav-woocommerce menu-item nav-item" itemtype="https://www.schema.org/SiteNavigationElement"><a href="' . wc_get_cart_url() . '" title="Shopping Cart" class="nav-link"><div class="view-cart d-inline-block">' . __('View Cart', 'c9-togo') . '</div> <i class="fa fa-shopping-cart fa-md"></i><span class="count">' . $count . '</span></a></li>';
			} else { //if not just put in an icon
				$items .= '<li itemscope="itemscope" class="nav-woocommerce menu-item nav-item" itemtype="https://www.schema.org/SiteNavigationElement"><a href="' . wc_get_cart_url() . '" title="Shopping Cart" class="nav-link"><div class="view-cart d-inline-block">' . __('View Cart', 'c9-togo') . '</div> <i class="fa fa-shopping-cart fa-md"></i></a></li>';
			} //end count check
